...
1. **Chapter Overview**（5–8 min）— 词汇闪卡预习 + 背景导读 + Reading / Quiz 状态卡片
...
本章目标词以**闪卡**形式预习，替代原先单面词汇卡片 +「Practice」按钮。

| 项目 | 说明 |
|------|------|
| 词表来源 | 自适应选词（`selectVocabularyForUser`），与 Quiz Section 2 目标词一致 |
| 正面 | **英文单词** + 🔊（点击发音，与 Review 页一致）；词性 |
| 背面 | **中文释义**（`definitionZh`；缺省时回退英文 `definition` 并附注）+ **书中例句**（`example`，**高亮目标词**） |
| 翻面 | **点击卡片**即可翻面 |
| **I know it** | 标记掌握，计入进度条；该词不再进入本轮队列（正面/背面均可点击） |
| **Show it later** | 将当前卡移至队列末尾，本轮继续；全部过一遍后，未掌握的词进入下一轮 |
| 完成条件 | 所有词均标记「已经会了」→ 自动进入 **Mini Check** |
| 进度条 | `Words learned` = 已掌握数 / 本章词表总数 |

**轮次逻辑**

1. 初始队列 = 本章全部目标词（顺序或自适应顺序）
2. 用户逐张翻面学习；「稍后再显示」的词在本轮末尾再次出现
3. 当队列为空且仍有未掌握词时，开启 **Round N+1**，仅包含未标记「已经会了」的词
4. 重复直至 `known === total`，触发 Mini Check

```
┌─────────────────────────────────────────┐
│  Card 3 of 12 · Round 2    8/12 known   │
├─────────────────────────────────────────┤
│              🔤                           │
│           peculiar 🔊                     │
│            adjective                    │
...
| JSON 字段 | 用途 |
|-----------|------|
| `definition` | 英文释义（Quiz / 家长端备用） |
| `definitionZh` | 闪卡背面中文释义（推荐每词填写） |
| `word` | 闪卡正面显示词；动词用原形、名词用单数 |
| `formInText` | 可选；例句中的实际词形（如 `winked`），用于例句高亮；与 `word` 不同时必填 |
| `example` | 书中例句；前端按 `formInText`（若有）或 `word` 做大小写不敏感高亮 |

### 背景导读

### 快捷跳转：Go to Reading

概览页右上角提供 **「Go to Reading →」** 链接，允许跳过剩余闪卡和 Mini Check，直接进入阅读计时页。
